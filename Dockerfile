...
RUN cd htslib-1.5 && ./configure --prefix=/home/software/htslib && make && make install
...
RUN cd samtools-1.5 && ./configure --prefix=/home/software/samtools && make && make install
...
RUN cd software && wget https://github.com/primer3-org/primer3/archive/refs/tags/v2.3.7.tar.gz
RUN cd software && tar xvf v2.3.7.tar.gz && rm v2.3.7.tar.gz
RUN cd software/primer3-2.3.7/src && make CFLAGS="-fpermissive"
...
ENV PATH="/home/software/primer3-2.3.7/src:$PATH"
